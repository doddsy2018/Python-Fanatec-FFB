...
# ──────────────────────────────────────────────
# 10. MPC STEERING CONTROLLER
# ──────────────────────────────────────────────

class MPCController:
    """
    Unconstrained linear MPC for wheel angle tracking.

    State  x = [angle_error, angular_velocity].
    Input  u = force_level (constant-force SDL effect value).
    Model (double integrator, error-state form):
        e[k+1] = e[k] + dt * v[k]
        v[k+1] = v[k] + (dt / tau) * u[k]

    The optimal gain is solved offline at construction via the batch QP;
    online compute is a single matrix-vector multiply.
    Output is clamped to [output_min, output_max].
    """

    def __init__(self, dt: float, horizon: int = 10,
                 q_error: float = 1.0, q_vel: float = 0.0001,
                 r_control: float = 1e-7, tau: float = 10.0,
        ...
        # Batch prediction matrices  Sx (nN×n),  Su (nN×mN)
        Sx = np.zeros((n * N, n))
        Su = np.zeros((n * N, m * N))
        Ak = np.eye(n)
        for k in range(N):
        ...
        Q_bar = np.kron(np.eye(N), np.diag([q_error, q_vel]))
        R_bar = r_control * np.eye(m * N)

        H = Su.T @ Q_bar @ Su + R_bar
        # Precompute first row of gain: u*[0] = -K_first @ e0
        self._K = np.linalg.solve(H, Su.T @ Q_bar @ Sx)[0:m].ravel()  # shape (n,)

    def reset(self):
        self._vel      = 0.0
        self._prev_ang = None

    def compute(self, error: float, current_angle: float, dt: float) -> float:
        """Return optimal force given angle error and current wheel position."""
        if self._prev_ang is not None and dt > 1e-6:
            self._vel = (current_angle - self._prev_ang) / dt
        self._prev_ang = current_angle
        u = float(-self._K @ np.array([error, -self._vel]))
        return max(self.output_min, min(self.output_max, u))


class AngleHoldController:
    """
    MPC wheel-position controller that drives to a target angle and holds it
    ...
    def __init__(self, di: FanatecDirectInput, js,
        ...
        self._mpc_params = dict(dt=dt, horizon=horizon, q_error=q_error,
                                q_vel=q_vel, r_control=r_control, tau=tau)
    ...
    def _loop(self):
        mpc    = MPCController(**self._mpc_params)
        print(f"[MPC] K={mpc._K}")
        t_prev = time.time()
        dbg_n  = 0
        ...
        try:
            while not self._stop_event.is_set():
                pygame.event.pump()
                raw_axis    = self._js.get_axis(self._axis)
                current_deg = raw_axis * self._half
                ...
                force = int(mpc.compute(error, current_deg, dt))
                #if dbg_n < 10 or dbg_n % 60 == 0:
                    #print(f"[MPC] target={target:.1f} axis={raw_axis:.3f} cur={current_deg:.1f} err={error:.1f} vel={mpc._vel:.1f} force={force}")
                dbg_n += 1
...
def demo():
    ...
    ctrl = AngleHoldController(di, js, steering_range=900.0, poll_hz=120.0)
